    ...
    public function index()
    {
        ...
        try {
            ...
            foreach ($contratos as $i => $contrato) {
                ...
                if ($contratos[$i]['plano_assinatura'] && isset($contratos[$i]['plano_assinatura']->data_cancelamento) &&  $contratos[$i]['plano_assinatura']->data_cancelamento != null) {
            ...
            if ($this->usuario_logado->tipo == "afiliado") {
                ...
                $categoriasAfiliado = AfiliadoCategorium::where("afiliado_id", $this->usuario_tipo_id)->where("status", "aprovado")->pluck("categoria_id", "id");
                $regioesAfiliado = AfiliadoRegiao::where("afiliado_id", $this->usuario_tipo_id)->pluck("regiao_id", "id");

                $orcamentos_aux11 = Orcamento::whereIn("categoria_id", $categoriasAfiliado)->whereIn("regiao_id", $regioesAfiliado)->where("afiliado_id", "=", null)->whereIn("status", [UtilStatusOrcamento::$ANALISANDO_CANDIDATOS, UtilStatusOrcamento::$ANALISANDO_ORCAMENTOS, UtilStatusOrcamento::$CANCELADO_PELO_ADMIN, UtilStatusOrcamento::$CANCELADO_PELO_AFILIADO, UtilStatusOrcamento::$CANCELADO_PELO_SINDICO])->orderBy("id", "ASC")->get();
                ...
                foreach ($orcamentos_aux11 as $v) {
                    $orcamentos_aux[] = OrcamentoBO::transformList($v, $this->usuario_logado->tipo, $this->usuario_tipo_id);
                }
                ...
                //     $orcamentos_aux[] = OrcamentoBO::transformList($orcamentos_aux33[$i], $this->usuario_logado->tipo, $this->usuario_tipo_id);
                // }
                        ...
                        if (isset($regAfil->plano_assinatura_afiliado_regiao_id)) {
                            ...
                            if ($planoRegiao) {
                                ...
                                if ($orc->afiliado_id == $this->usuario_tipo_id || ($autorizeAsaas && $autorizeAutentique && $usuarioApp && $usuarioApp->data_confirmacao && $isInadimplente == false)) {
                                    ...
                                    if ($this->usuario_logado->tipo == "afiliado") {
                                        if (($orc['interesse'] == null && $orc['status'] == 1) || $this->usuario_tipo_id == $orc->afiliado_id) {
                                            if ($orc->modo == Util::getModusOperandi()) {
                                                $orcamentos[$orc->id] = $orc;
                                            }
                                        } else if ($orc['interesse'] != null && $orc['interesse']->descartado_sindico != 1 && $orc['interesse']->descartado_afiliado == 0) {
                                            if ($orc->modo == Util::getModusOperandi()) {
                                                $orcamentos[$orc->id] = $orc;
                                            }
                                        }
            ...
            $orcamentosResult = [];
            foreach ($orcamentos as $orcamentoLinha) {
                $orcamentosResult[] = $orcamentoLinha;
            }
            return $this->successResponse("ok", $orcamentosResult);
        } catch (Exception $e) {
    ...
    public function indexPaginate($page)
    {
        ...
        try {
            ...
            if ($this->usuario_logado->tipo == "afiliado") {
                ...
                OrcamentoBO::transformList($v, $this->usuario_logado->tipo, $this->usuario_tipo_id);
                $orcamentos_aux = [];
                for ($i = ($page - 1) * $quantidade_itens_pagina; $i < ($page * $quantidade_itens_pagina); $i++) {
                    $orcamentos_aux[] = OrcamentoBO::transformList($orcamentos_aux22[$i], $this->usuario_logado->tipo, $this->usuario_tipo_id);
                }
